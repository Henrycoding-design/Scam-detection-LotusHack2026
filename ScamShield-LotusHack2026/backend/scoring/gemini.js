...
export function buildOpenAIExplanationRequest({
  ...
  return {
    ...
    input: `You are a cybersecurity assistant helping a non-technical person understand whether a website looks like a scam.

Analyze the context below and return a plain-language explanation that matches the required JSON schema.
...
${topSignals || "- No major heuristic signals provided"}
...
- Fill risk_location with where the scam is most likely happening.
- prevention_tips must contain 2-3 short tips.
- Recommend one immediate next action.
- If the page does not look risky, verdict may be "safe".`,
    text: {
      format: {
        type: "json_schema",
        name: "scamshield_backend_explanation",
        strict: true,
...
export async function getOpenAIExplanation(context, fetchImpl = globalThis.fetch) {
  if (context.score < 30 || !process.env.OPENAI_API_KEY) {
    return null;
  }
  ...
  try {
    ...
    console.warn("[ScamShield] Backend OpenAI parse error:", error);
    return null;
  }
}

export async function getGeminiExplanation(context, fetchImpl = globalThis.fetch) {
  return getOpenAIExplanation(context, fetchImpl);
}
...
function extractParsedExplanation(data) {
  const candidates = [data?.output_parsed, data?.parsed];
